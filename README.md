...
> [!IMPORTANT]
> 本章保留通用写作表格：每个部分先回答“这一节要解决什么问题、逻辑是什么、读者要带走什么”。随后以 `Learning Diffusion Priors for Inverse Rendering Under Unknown Illumination` / `IntrinsicAnything` 为例，拆解具体内容。反例不是来自真实论文，而是由正例反推出来的常见错误写法。
...
| 聚焦点 | 论文中的具体内容 | 通俗解释 |
|---|---|---|
| 标题抓住关键条件 | `Under Unknown Illumination` | 这几个词很重要：如果光照已知，问题简单很多；未知光照才是这篇论文的难点和卖点 |
| 摘要先讲任务 | 从 posed images 中恢复 object materials | 新人要学会先说输入输出：给多视角图像，想恢复物体材质 |
| 摘要讲清为什么难 | 几何、材质、环境光耦合，导致 inverse rendering 有歧义 | 一张图的颜色可能来自材质，也可能来自光照或阴影；如果不拆清楚，优化会走错 |
| 摘要讲方法不是堆术语 | 把 material prior 设计成 albedo 和 specular 的 diffusion models | 不是“用了 diffusion 所以高级”，而是 albedo/specular 正好对应材质分解中的两类关键因素 |
...
| `Existing methods are inaccurate. We propose a diffusion model.` | gap 没有解释为什么不准，method 和 problem 没有扣上 | 写清：因为 shading/shadow 可能被误分解成 albedo，所以需要 material prior 约束 |
...
| 聚焦点 | 论文中的具体内容 | 通俗解释 |
|---|---|---|
| Fig. 1 的作用 | 展示两类歧义：diffuse shading 与 albedo 的歧义；shadow 与 albedo 的歧义 | 这是非常好的问题图：新人要学会把 gap 画出来，而不是只写出来 |
| 黄色光照例子 | Xbox 被黄色光照照亮，baseline 容易把黄色光照 baked into albedo | 读者一看就懂：物体本来不一定黄，但算法把灯光颜色当成材质颜色了 |
| 自遮挡阴影例子 | 瓷玩具有自遮挡，baseline 把 shadow baked into recovered albedo | 阴影本来是光照现象，不应该变成材质纹理；这就是分解错误 |
| 方法动机 | 学习 albedo 和 specular 的 generative priors | 既然优化容易错分解，就用“正常材质应该长什么样”的先验拉住它 |
| Introduction 的推进 | 应用价值 → 旧采集限制 → 当前优化路线 → 具体歧义 → material prior | 这是一条完整链条，读者不会觉得方法突然冒出来 |
...
| 聚焦点 | 论文中的具体内容 | 通俗解释 |
|---|---|---|
| Fig. 3 pipeline | lighting、geometry、roughness、albedo 组合渲染 RGB 和 specular images，两阶段优化 | 方法图不是装饰，它告诉读者：从图像到材质恢复的完整路径是什么 |
| rendering equation 分解 | 把渲染拆成 diffuse term 和 specular term | 这一步很关键：作者不是随便学两个 prior，而是从物理渲染方程推出这两个 prior |
| 不直接学 roughness prior | roughness 不容易观察，很多数据集没有可靠 roughness | 好的 Method 会解释“为什么不那么做”，这能增强设计可信度 |
| 学 specular shading prior | specular shading 和 roughness 相关，而且更容易从合成对象生成监督 | 这是把不可直接学的量转成更可观察的量，新人要学这种设计解释 |
| coarse-to-fine | 第一阶段得到 coarse material，第二阶段用它 guide diffusion，增强多视角一致性 | 不是简单两阶段，而是第一阶段的结果服务于第二阶段的约束 |
| high-resolution patch guidance | 用 multiple-measurement DPS 处理 patch 与粗材质、相邻 patch 的一致性 | 当图像太大不能整张进 diffusion，就要解释 patch 怎么避免接缝不一致 |

#### 由正例反推的反例

| 不推荐写法 | 问题 | 如何改 |
|---|---|---|
| “Our method has a renderer, a diffusion model, and an optimizer.” | 像模块清单，缺少设计论证 | 按问题写：rendering loss 有歧义，所以引入 material priors；prior 分成 albedo/specular 是由 rendering equation 推出 |
| “We use specular prior because it improves performance.” | 只说结果，不说原因 | 解释 roughness 难观察，所以用 specular shading 作为更可监督、和 roughness 相关的替代对象 |
| “We use two-stage optimization for stable training.” | stable 太虚 | 写清第一阶段产生 coarse materials，第二阶段用它们 guide diffusion 生成多视角一致约束 |
...
| prior 是否真有用 | w/o prior 或替换 prior 的消融 | 证明提升不是来自渲染器或优化细节，而是来自 material prior |
...
| 聚焦点 | 论文中的具体内容 | 通俗解释 |
|---|---|---|
| Table I | synthetic dataset 上比较 albedo、roughness、relighting 等指标；包含 w/o guided、替换 prior 等变体 | 一张主表既回答“强不强”，也开始回答“为什么强” |
| Table IV | 把 diffusion prior 加到 TensoIR、NvdiffrecMC 等 baseline 上 | 这个实验很有说服力：说明 prior 是通用插件，不只服务于作者自己的框架 |
| Fig. 13 | ablation 展示 w/o spec、w/o albedo、w/o guided 的具体失败 | 好的消融不仅有数字，还要展示失败长什么样 |
| w/o albedo prior | recovered albedo 出现 baked-in shading | 这直接对应 Introduction 里的 shadow-albedo ambiguity，story 前后闭合 |
| w/o specular prior | roughness 和 lighting 估计错误，relighting 不准 | 这说明 specular prior 不是可有可无的模块 |
| Fig. 16 | joint geometry optimization 会带来 rough surfaces | 这是很好的边界说明：方法受几何质量影响，未来可做 geometry prior |
...
| 只在 synthetic data 上验证 | unknown illumination 和真实材质复杂性没有被充分验证 | 加 real-world data、relighting、qualitative decomposition |
| 只展示好看的 relighting 图 | 容易被认为挑样例 | 加同场景 baseline 对比、失败样例、定量指标 |
| ablation 只写 variant A/B/C | reviewer 不知道变体对应哪个设计假设 | 变体名直接写设计：w/o albedo prior、w/o specular prior、w/o guided |
...
| 聚焦点 | 论文中的具体内容 | 通俗解释 |
|---|---|---|
| Related Work 分类 | learning priors for inverse rendering；inverse rendering with neural representation；diffusion models | 三类正好对应本文坐标：inverse rendering 任务、neural/PBR 优化路线、diffusion prior 工具 |
| 差异表达 | 不是否定前人，而是指出仅靠 rendering loss 难以解除材质/光照歧义 | 好的 Related Work 不是说别人都不行，而是说明自己补哪块短板 |
| Conclusion 回到主线 | material prior 被设计成 albedo 和 specular shading 的 conditional diffusion models | 结论没有散，仍然围绕方法主线 |
| Conclusion 写具体贡献 | two-stage training、DPS、多 guidance signals、high-resolution patch consistency | 结论不只是“有效”，而是概括本文真正贡献点 |
| 边界意识 | joint geometry optimization 会导致 rough surfaces；几何质量会影响材质恢复 | 局限写得具体，未来方向自然：geometry prior |

#### 由正例反推的反例

| 不推荐写法 | 问题 | 如何改 |
|---|---|---|
| “Many works study inverse rendering. Some use neural rendering and some use diffusion.” | 文献流水账，没有坐标系 | 按路线组织：prior learning、neural inverse rendering、diffusion for inverse problems |
| “Our method is different from all previous methods.” | 过度割裂，显得不理解前人 | 写成：本文继承 differentiable PBR 优化框架，但把贡献放在 material diffusion prior 上 |
| “Future work will improve the method.” | 没有边界信息 | 写清：几何误差会影响材质恢复，因此未来可学习 geometry prior |
